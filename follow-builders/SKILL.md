...
### Example 1: X/Twitter Signals Digest (Bilingual)
...
Agents are now handling 80% of internal code review at OpenAI. The remaining 20% is edge cases where human judgment still matters — mostly around security-sensitive diffs.
https://x.com/sama/status/1932001234567890

Sam Altman 指出 OpenAI 內部 80% 嘅 code review 已經由 agent 處理，剩餘 20% 係需要人類判斷嘅邊緣案例，主要涉及 security-sensitive diffs。
https://x.com/sama/status/1932001234567890

---

**Andrej Karpathy** (@karpathy) — Eureka Labs founder
Wrote a 12-part thread on why "LLM OS" is the wrong mental model. The right framing: LLMs are the CPU, but the motherboard (context window), RAM (memory), and I/O (tool use) matter more for real-world performance.
https://x.com/karpathy/status/1932009876543210

Andrej Karpathy 用 12 條 thread 解釋點解「LLM OS」係錯誤嘅心智模型。正確嘅框架係：LLM 係 CPU，但係 motherboard（context window）、RAM（memory）同 I/O（tool use）對實際表現影響更大。
https://x.com/karpathy/status/1932009876543210

---

**Mira Murati** (@miramurati) — Thinking Machines Lab CEO
Our first model checkpoint hits GPT-4 level reasoning at 1/10th the inference cost. Not through distillation — through a new attention mechanism we'll publish next month.
https://x.com/miramurati/status/1932005551234567

Mira Murati 透露佢哋第一個 model checkpoint 以 1/10 推理成本達到 GPT-4 級別推理能力。唔係靠 distillation，而係透過新嘅 attention mechanism，下個月會公開。
https://x.com/miramurati/status/1932005551234567
```

### Example 2: YouTube Podcast Summary (English)
...
**Lex Fridman Podcast #482** — Dario Amodei (Anthropic CEO)

Dario makes the case that 2026 is the year AI safety goes from "theoretical concern" to "engineering discipline." Three key points:

1. **Constitutional AI at scale**: Anthropic's internal red-teaming now uses 10,000 synthetic personas to stress-test models — more diverse than any human team could be.

2. **The liability question**: He predicts that by 2027, AI companies will carry something analogous to product liability insurance. "If your agent breaks something, someone's paying for it."

3. **Open-source tension**: Dario is blunt — open-weight models are good for research but create a "tragedy of the commons" for safety. His proposed compromise: open weights for models below a capability threshold, gated access above it.

The most interesting moment: when Lex pushed on whether Anthropic's safety-first brand is just marketing, Dario paused for 6 seconds and said, "The market rewards speed. We leave money on the table every day. That's not marketing — that's a bet."

Full episode: https://youtube.com/watch?v=example123
```

### Example 3: Mixed Sources Day (Bilingual)
...
Replit Agent 4 just shipped. It can now scaffold a full-stack app, deploy it, and iterate on user feedback — all from a single prompt. We're seeing 3x conversion on the free tier.
https://x.com/amasad/status/1932007778889999

Replit CEO Amjad Masad 發佈 Agent 4，可以由一個 prompt 開始 scaffold 全棧應用、部署、再根據用戶反饋迭代。免費層轉化率提升 3 倍。
https://x.com/amasad/status/1932007778889999

---

**Guillermo Rauch** (@ra) — Vercel CEO
v0 now generates and deploys entire Next.js apps in under 30 seconds. The bottleneck shifted from generation to DNS propagation.
https://x.com/ra/status/1932003334445555

Vercel CEO Guillermo Rauch 宣佈 v0 可以喺 30 秒內生成同部署完整 Next.js 應用，瓶頸已經由代碼生成轉移到 DNS propagation。
https://x.com/ra/status/1932003334445555
...
**The Cognitive Revolution** — Nathan Labenz interviews Percy Liang (Stanford HELM)

Percy argues that benchmarks are broken — not because they're too easy, but because they measure the wrong thing. "MMLU tells you what a model memorized. It tells you nothing about what it can do."

Key takeaways:
- HELM's new "behavioral benchmarking" tests what models actually do in production-like scenarios
- Early results show a 40% gap between benchmark scores and real-world task completion
- Stanford is releasing an open leaderboard that tracks deployment-relevant metrics

Most provocative claim: "The companies winning benchmarks will lose the market. The companies winning users will publish fewer papers."

Full episode: https://youtube.com/watch?v=example456
```
...
## Integration
- Content collected by this skill is consumed by `/signal-recommend` for cold-start signal digests
