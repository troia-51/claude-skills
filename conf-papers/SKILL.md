...
## Follow-up Skills
- After finding papers, use `/paper-analyze` for deep analysis
- Use `/extract-paper-images` to get paper figures

## Good Output Examples

### Example 1: Search with Results (CVPR 2024, Top 3 with Deep Analysis)

User runs `/conf-papers 2024 CVPR`. The skill produces a note at `10_Daily/2024_顶会论文推荐.md`:

```yaml
---
keywords: [large language model, LLM, multimodal, vision-language, grounding, instruction tuning, diffusion, generation]
tags: ["llm-generated", "conf-paper-recommend"]
---
```
...
本次从 **CVPR** 中共搜索到 2,847 篇论文，经过研究兴趣匹配筛选出 186 篇候选，最终推荐以下 10 篇高质量论文。

- **总体趋势**：多模态大模型（Multimodal LLM）持续主导，视觉-语言对齐（Vision-Language Alignment）和指令微调（Instruction Tuning）成为主流范式。扩散模型（Diffusion Models）从图像生成扩展到视频和 3D。

- **研究热点**：
  - **Multimodal Grounding**：让模型精确定位图像中的物体并回答空间关系问题
  - **Efficient Fine-tuning**：LoRA、QLoRA 等参数高效微调方法在大模型上的应用
  - **Video Understanding**：从单帧理解扩展到长视频时序推理

- **阅读建议**：建议从 #1 和 #2 开始（多模态基础），再看 #3（效率优化），其余按兴趣选读

---

### [[LLaVA-NeXT-Improved-Reasoning-Occlusion-Relief|LLaVA-NeXT: Improved Reasoning, OCR, and World Knowledge]]
- **Authors**: Haotian Liu, Chunyuan Li, Yuheng Li, Bo Li, Yuanhan Zhang, Sheng Shen, Yong Jae Lee
- **Affiliation**: UW-Madison, Microsoft Research, ByteDance
- **Conference**: CVPR 2024
- **Citations**: 1,247 (influential: 89)
- **Links**: [DBLP](https://dblp.org/rec/conf/cvpr/Liu0LL0ZSL024) | [arXiv](https://arxiv.org/abs/2401.04024) | [PDF](https://arxiv.org/pdf/2401.04024)
- **详细报告**：[[20_Research/Papers/Foundation Models & LLM/LLaVA-NeXT-Improved-Reasoning-Occlusion-Relief]] (自动生成)

**一句话总结**：通过 AnyRes 策略和改进的视觉编码，在不增加训练成本的情况下显著提升多模态推理和 OCR 能力。

![论文图片|600](assets/LLaVA-NeXT-Improved-Reasoning-Occlusion-Relief/fig1.png)

**核心贡献/观点**：
- 提出 AnyRes 策略，动态分割高分辨率图像为多个子图，提升细粒度理解
- 在不改变训练数据的情况下，仅通过架构改进提升 10%+ 的 OCR 性能
- 证明视觉编码器的分辨率是多模态推理的关键瓶颈

**关键结果**：在 12 个 benchmark 上超越前代 LLaVA，OCR 任务提升 15.2%，数学推理提升 8.7%

---

### [[VideoChat-Chat-Centric-Video-Understanding|VideoChat: Chat-Centric Video Understanding]]
- **Authors**: KunChang Li, Yinan He, Yi Wang, Yizhuo Li, Wenhai Wang, Ping Luo, Yali Wang, Limin Wang, Yu Qiao
- **Affiliation**: Shanghai AI Lab, Nanjing University, University of Hong Kong
- **Conference**: CVPR 2024
- **Citations**: 456 (influential: 23)
- **Links**: [DBLP](https://dblp.org/rec/conf/cvpr/Li0HWLW0LW024) | [arXiv](https://arxiv.org/abs/2306.05424) | [PDF](https://arxiv.org/pdf/2306.05424)
- **详细报告**：[[20_Research/Papers/Foundation Models & LLM/VideoChat-Chat-Centric-Video-Understanding]] (自动生成)

**一句话总结**：构建了以对话为中心的视频理解框架，结合时序建模和大语言模型实现开放式视频问答。

![论文图片|600](assets/VideoChat-Chat-Centric-Video-Understanding/fig1.png)

**核心贡献/观点**：
- 提出 VideoChat 框架，将视频理解从分类/检索扩展到开放式对话
- 设计时序感知的视觉编码器，捕捉视频中的动作和事件序列
- 构建了大规模视频对话数据集（110K 视频-对话对）

**关键结果**：在 MVBench 上超越现有方法 12.4%，在 VideoChatGPT benchmark 上人类评估胜率 78%

---

### [[Efficient-Visual-Instruction-Tuning-LoRA|Efficient Visual Instruction Tuning with LoRA]]
- **Authors**: Artidoro Pagnoni, Saurav Kadavath, Alex Mei
- **Affiliation**: University of Washington
- **Conference**: CVPR 2024
- **Citations**: 312 (influential: 15)
- **Links**: [DBLP](https://dblp.org/rec/conf/cvpr/PagnoniKM24) | [arXiv](https://arxiv.org/abs/2403.01234) | [PDF](https://arxiv.org/pdf/2403.01234)
- **备注**：无 arXiv 版本，无法自动提取图片

**一句话总结**：系统评估了 LoRA 在多模态大模型微调中的有效性，发现仅微调 0.1% 参数即可达到全量微调 95% 的性能。

**核心贡献/观点**：
- 首次系统比较 LoRA vs. 全量微调在多模态模型上的表现
- 发现视觉编码器的 LoRA rank 需要高于语言模型部分
- 提出分层 LoRA 策略：不同层使用不同的 rank

**关键结果**：训练成本降低 70%，显存占用减少 60%，性能损失仅 3-5%

... (remaining 7 papers follow the same format)
```

### Example 2: Deep Analysis of a Specific Paper

After the search results are generated, the top 3 papers with arXiv IDs trigger deep analysis. For each, the skill calls `extract-paper-images` and `paper-analyze`:

```
# Deep analysis triggered for: LLaVA-NeXT

## Step 1: Check existing notes
Searching 20_Research/Papers/ for "LLaVA-NeXT"...
→ No existing notes found.

## Step 2: Extract images
Running /extract-paper-images 2401.04024...
→ Found arXiv source package
→ Extracted 6 figures to assets/LLaVA-NeXT-Improved-Reasoning-Occlusion-Relief/
  - fig1.png (architecture overview)
  - fig2.png (AnyRes strategy)
  - fig3.png (qualitative comparison)
  - fig4.png (OCR benchmark)
  - fig5.png (ablation study)
  - fig6.png (failure cases)

## Step 3: Generate detailed report
Running /paper-analyze 2401.04024...
→ Created 20_Research/Papers/Foundation Models & LLM/LLaVA-NeXT-Improved-Reasoning-Occlusion-Relief.md
→ Report includes:
  - Problem statement and motivation
  - Method overview with architecture diagram
  - Key experiments and ablation studies
  - Comparison with prior work
  - Limitations and future directions
  - Personal assessment and relevance score

## Step 4: Update recommendation note
→ Added wikilink to detailed report in recommendation note
→ Embedded first figure as preview image
→ Note saved to: 10_Daily/2024_顶会论文推荐.md
```

### Example 3: No Results Found (Edge Case)

User runs `/conf-papers 2023 ICCV` in an odd year. ICCV is biennial (odd years only).

```
# Searching ICCV 2023...

## Results
- DBLP search: 0 papers found for ICCV 2023
- Reason: ICCV is a biennial conference held in odd years (2021, 2023, 2025, ...)
- The search year 2023 has no ICCV proceedings

## Action
→ Skipped ICCV (biennial conference, no 2023 proceedings)
→ Note: Use even years (2024, 2026) for ICCV searches

## Partial results from other configured conferences
If other conferences were also configured (e.g., NeurIPS 2023), those results are still included:

### NeurIPS 2023 Results
(total_found: 3,241 | filtered: 203 | top 10 recommended)

[... papers listed in standard format ...]
```
